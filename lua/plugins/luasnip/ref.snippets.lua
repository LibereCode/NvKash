...
ls.add_snippets('all', {
  ...
  s({ trig = 'a%d', regTrig = true }, f(function(_, snip) return 'Triggered with ' .. snip.trigger .. '.' end, {})),
  -- It's possible to use capture-groups inside regex-triggers.
  s({ trig = 'b(%d)', regTrig = true }, f(function(_, snip) return 'Captured Text: ' .. snip.captures[1] .. '.' end, {})),
  s({ trig = 'c(%d+)', regTrig = true }, {
    t 'will only expand for even numbers',
  }, {
    condition = function(line_to_cursor, matched_trigger, captures) return tonumber(captures[1]) % 2 == 0 end,
  }),
